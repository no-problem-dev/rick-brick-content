...
Constitutional AIの概念は、2022年12月にAnthropicが発表した論文「Constitutional AI: Harmlessness from AI Feedback」（arXiv:2212.08073）で初めて体系的に提示された。著者はYuntao Bai他50名の共著者による大規模な共同研究だ。
...
原論文の実験結果では、Constitutional AIを適用したモデルは従来のRLHFベースのモデルと同等か、それ以上の安全性を示した。特筆すべきは「有害性が低く、かつ回避的でない」という特性だ。

従来の安全性フィルタリングは「危険なクエリを拒否する」という単純なアプローチが多かった。結果として、過剰に拒否する（false positive が多い）か、それとも通過させすぎる（false negative が多い）かのどちらかに偏りがちだった。Constitutional AIでは、モデルが「なぜこれが問題か」を理解した上で応答するため、文脈に応じた適切な判断が可能になる。
...
また、ハードコンストレイント（生物兵器の製造支援など絶対的な禁止事項）は引き続き明示されているが、大部分の指針は「判断力を育てる」ことに焦点を当てている。
...
Anthropicは文書中で「意図されたふるまいと実際のモデルの振る舞いの間にはギャップがある」と率直に認めており、評価の継続と安全性研究の拡充をコミットしている。
...
Anthropicは複数の安全性研究方向を並行追求しており、解釈可能性（Interpretability）・スケーラブルな監視・プロセス指向学習・汎化理解などが主要な柱となっている。Constitutional AIはこれらの研究の中で「実装に最も近い」部分に位置付けられる。

Constitutional AI論文の発表（2022年）→ 初期憲法の公開（2023年）→ 改訂版憲法のCC0公開（2026年1月）という流れは、研究→実践→業界標準化という段階的な影響力拡大のシナリオを見せている。

```mermaid
timeline
    title Constitutional AI の進化
    2022 : 原論文発表（arXiv:2212.08073）
         : RLAIF手法の提案
    2023 : 初期憲法の公開
         : ルールリスト形式
    2026 : 改訂版憲法のCC0公開
...
| Constitutional AI: Anthropicの安全性研究 | Anthropic Research | 2023 | https://www.anthropic.com/research/constitutional-ai-harmlessness-from-ai-feedback |
