...
async function generateThumbnail(
  ...
  try {
    const prompt = `Shot on Canon EOS R5 with RF 50mm f/1.2L lens, ISO 400, natural window lighting. A real domestic cat photographed in a scene related to the blog topic.\nTopic: ${title}\nSummary: ${summary}\nPhotography direction: This must look like an actual photograph taken with a professional DSLR camera — real fur texture with individual hair strands visible, natural eye reflections and catchlights, shallow depth of field with creamy bokeh background, realistic ambient lighting with soft shadows. The cat should be a real breed (tabby, calico, Scottish Fold, etc.) naturally posed in or near props that represent the topic. For example: if the topic is about AI models, a cat sitting on a desk next to a laptop with code on screen; if about robotics, a cat curiously pawing at small mechanical parts on a workbench; if about a tech company announcement, a cat lounging on a newspaper/tablet. The props and environment must also look photographically real — no CGI, no digital art, no illustration style. Think National Geographic cat photography meets tech lifestyle editorial.\nSTRICTLY PROHIBITED: Do NOT include any text, letters, words, numbers, watermarks, logos, or captions anywhere in the image. The image must contain absolutely zero text or writing of any kind. No cartoon or illustrated style — this must be indistinguishable from a real photograph.`;
