...
import { ARTICLES_DIR, IMAGES_DIR } from '../config/constants.js';
...
const THUMBNAIL_COMMON_CONSTRAINTS = `COMPOSITION: Do NOT place the cat in the center facing the camera. Use compositions inspired by famous paintings (Vermeer's lighting, Kubrick's one-point perspective, Wes Anderson's symmetry), iconic movie scenes, or historical moments. Vary the angle every time — low angle, bird's eye, Dutch angle, over-the-shoulder, side profile, rear view. The cat may wear costumes, hats, capes, tiny props, or accessories appropriate to the parody scene.
The cat itself must look like an actual photograph — real fur texture with individual hair strands visible, natural eye reflections and catchlights, realistic ambient lighting with natural shadows. However, the SITUATION must be completely impossible and surreal — a real cat placed in an absurd, fantastical, or physically impossible scenario. The contrast between "photorealistic cat" and "impossible situation" is the core aesthetic.
